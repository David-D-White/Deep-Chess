...
public class Search {
	...
	public interface Listener {
		...
		public void notifyPV(int depth, int score, int time, int nodes, int nps, boolean isMate, boolean upperBound, boolean lowerBound, ArrayList<Move> pv);
	...
	final public Move iterativeDeepening(MoveGen.MoveList scMovesIn, int maxDepth, int initialMaxNodes, boolean verbose) {
		...
		try {
			...
			for (depth = 1;; depth++) {
				...
				for (int mi = 0; mi < scMoves.length; mi++) {
					...
					if (score >= beta) {
						...
						while (score >= beta) {
							...
							bestMove = m;
							if (verbose)
								System.out.printf("%-6s %6d %6d %6d >=\n", TextIO.moveToString(pos, m, false), score, nodes, qNodes);
					...
					if (verbose || ((listener != null) && (depth > 1))) {
						...
						if (verbose) {
							 ...
							 * System.out.printf("%-6s\n", TextIO.moveToUCIString(m));
							 */
							System.out.printf("%-6s %6d %6d %6d%s %s\n", TextIO.moveToString(pos, m, false), score, nodes, qNodes, (score > alpha ? " *" : ""), PV);
						}
						if (havePV && (depth > 1)) {
				...
				if (verbose) {
					...
					System.out.printf("Time: %.3f depth:%d nps:%d\n", (tNow - tStart) * .001, depth, (int) (totalNodes / ((tNow - tStart) * .001)));
				}
				if (maxTimeMillis >= 0) {
	 ...
	 */
	final public int negaScout(int alpha, int beta, int ply, int depth, int recaptureSquare, final boolean inCheck) throws StopSearch {
		if (log != null) {
		...
		if (--nodesToGo <= 0) {
			...
			if (((timeLimit >= 0) && (tNow - tStart >= timeLimit)) || ((maxNodes >= 0) && (totalNodes >= maxNodes))) {
		...
		if (canClaimDraw50(pos)) {
			if (MoveGen.canTakeKing(pos)) {
				int score = MATE0 - ply;
				if (log != null)
					log.logNodeEnd(searchTreeInfo[ply].nodeIdx, score, TTEntry.T_EXACT, UNKNOWN_SCORE, hKey);
				return score;
			}
			if (inCheck) {
			...
			if (log != null)
				log.logNodeEnd(searchTreeInfo[ply].nodeIdx, 0, TTEntry.T_EXACT, UNKNOWN_SCORE, hKey);
			return 0;
		}
		...
		if (ent.type != TTEntry.T_EMPTY) {
			...
			if ((beta == alpha + 1) && ((eDepth >= depth) || (eDepth >= plyToMate * plyScale))) {
				if ((ent.type == TTEntry.T_EXACT) || (ent.type == TTEntry.T_GE) && (score >= beta) || (ent.type == TTEntry.T_LE) && (score <= alpha)) {
					if (score >= beta) {
		...
		if (depth + posExtend <= 0) {
			...
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, score, type, evalScore, hKey);
			return score;
		}
		...
		if ((depth >= 3 * plyScale) && !inCheck && sti.allowNullMove && (Math.abs(beta) <= MATE0 / 2)) {
			if (MoveGen.canTakeKing(pos)) {
				int score = MATE0 - ply;
				if (log != null)
					log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_EXACT, evalScore, hKey);
				return score;
			}
			boolean nullOk;
			if (pos.whiteMove) {
			...
			if (nullOk) {
				...
				if (score >= beta) {
					...
					if (log != null)
						log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_GE, evalScore, hKey);
					return score;
				} else {
					if ((searchTreeInfo[ply - 1].lmr > 0) && (depth < 5 * plyScale)) {
						...
						if (m1.from != m1.to) {
							if ((m1.to == m2.from) || (m1.from == m2.to) || ((BitBoard.squaresBetween[m2.from][m2.to] & (1L << m1.from)) != 0)) {
								...
								if (log != null)
									log.logNodeEnd(sti.nodeIdx, alpha, TTEntry.T_LE, evalScore, hKey);
								return alpha;
							}
						}
					}
		...
		if ((Math.abs(alpha) <= MATE0 / 2) && (depth < 4 * plyScale) && (beta == alpha + 1)) {
			...
			if (evalScore < beta - razorMargin) {
				...
				if (score <= alpha - razorMargin) {
					...
					if (log != null)
						log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_LE, evalScore, hKey);
					return score;
				}
			}
		}
		...
		for (int mi = 0; mi < moves.size; mi++) {
			...
			if (doFutility) {
				...
				if (((lmr > 0) && (score > alpha)) || ((score > alpha) && (score < beta) && (b != beta) && (score != illegalScore))) {
					sti.lmr = 0;
					newDepth += lmr;
			...
			if (alpha >= beta) {
				...
				moveGen.returnMoveList(moves);
				if (log != null)
					log.logNodeEnd(sti.nodeIdx, alpha, TTEntry.T_GE, evalScore, hKey);
				return alpha;
			}
			b = alpha + 1;
		}
		...
		if (bestMove >= 0) {
			...
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, bestScore, TTEntry.T_EXACT, evalScore, hKey);
		} else {
			emptyMove.score = bestScore;
			tt.insert(hKey, emptyMove, TTEntry.T_LE, ply, depth, evalScore);
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, bestScore, TTEntry.T_LE, evalScore, hKey);
		}
		moveGen.returnMoveList(moves);
		return bestScore;
	}
	...
	private final boolean weakPlaySkipMove(Position pos, Move m, int ply) {
		long rndL = pos.zobristHash() ^ Position.psHashKeys[0][m.from] ^ Position.psHashKeys[0][m.to] ^ randomSeed;
			...
			if (inCheck) {
				...
				if ((pos.getPiece(m.to) == Piece.EMPTY) && (m.promoteTo == Piece.EMPTY)) {
					...
					if (optimisticScore < alpha) { // Delta pruning
						if ((pos.wMtrlPawns > 0) && (pos.wMtrl > capt + pos.wMtrlPawns) && (pos.bMtrlPawns > 0) && (pos.bMtrl > capt + pos.bMtrlPawns)) {
							if (depth - 1 > -4) {
								givesCheck = MoveGen.givesCheck(pos, m);
								givesCheckComputed = true;
							}
	 ...
	 * 
	 * @return SEE score for m. Positive value is good for the side that makes the
	 *         first move.
	 */
	...
	/**
	 * Compute scores for each move in a move list, using SEE, killer and history
	 * information.
	 * 
	 * @param moves
	...
	/**
	 * If hashMove exists in the move list, move the hash move to the front of the
	 * list.
	 */
