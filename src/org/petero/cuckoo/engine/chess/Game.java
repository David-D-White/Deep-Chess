...
public class Game {
	...
	for (int i = 0; i < currentMove; i++) {
		...
		if (compressed) {
		    ret.append(String.format("%d. %s %s ", pos.fullMoveCounter, whiteMove, blackMove));
		}
		else {
		    ret.append(String.format("%3d.  %-10s %-10s%n", pos.fullMoveCounter, whiteMove, blackMove));
		}
		whiteMove = "";
		blackMove = "";
	    }
	...
	if ((whiteMove.length() > 0) || (blackMove.length() > 0)) {
		...
		ret.append(String.format("%3d.  %-8s %-8s%n", pos.fullMoveCounter, whiteMove, blackMove));
	    }
	}
	...
	if (pos.pieceTypeBB[Piece.WQUEEN] != 0)
	    return false;
	if (pos.pieceTypeBB[Piece.WROOK] != 0)
	    return false;
	if (pos.pieceTypeBB[Piece.WPAWN] != 0)
	    return false;
	if (pos.pieceTypeBB[Piece.BQUEEN] != 0)
	    return false;
	if (pos.pieceTypeBB[Piece.BROOK] != 0)
	    return false;
	if (pos.pieceTypeBB[Piece.BPAWN] != 0)
	    return false;
	...
	if (wn + bn == 0) {
	    ...
	    if (((bMask & BitBoard.maskDarkSq) == 0) || ((bMask & BitBoard.maskLightSq) == 0))
		return true;
	}
