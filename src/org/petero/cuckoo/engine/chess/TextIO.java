...
public class TextIO {
	...
	for (int i = 0; i < words[0].length(); i++) {
		...
		case 'k':
		    ...
		    break;
		default:
		    throw new ChessParseError("Invalid piece");
	    }
	}
	if (words[1].length() == 0) {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (i == 0) {
		    ...
		    if (strMove.toLowerCase().equals(str1.toLowerCase()) || strMove.toLowerCase().equals(str2.toLowerCase())) {
			return m;
		    }
		}
	    }
	...
	for (int i = 0; i < 2; i++) {
		...
		if (i == 0) {
		    ...
		    match = (str1.toLowerCase().startsWith(strMove.toLowerCase()) || str2.toLowerCase().startsWith(strMove.toLowerCase()));
		}
		if (match) {
		    if (move != null) {
	...
	ret.append("      aa   bb   cc   dd   ee   ff   gg   hh");
	ret.append(nl);
	ret.append("    +----+----+----+----+----+----+----+----+");
	ret.append(nl);
	for (int y = 7; y >= 0; y--) {
